...
\section{Polinomios sobre un DFU} % [cite: 303]

\begin{definicion}{Polinomio Primitivo y Contenido} % [cite: 304, 306]
    Sea $D$ un Dominio de Factorización Única (DFU) y $K$ su cuerpo de fracciones. Sea $P \in K[X] \setminus \{0\}$. % [cite: 303]
    \begin{itemize}
        \item Un polinomio $Q \in D[X]$ es un \textbf{polinomio primitivo} si sus coeficientes son coprimos entre sí. % [cite: 305]
        \item Llamaremos \textbf{contenido} de $P$ a un elemento $a \in K$ tal que $P = a P_1$ con $P_1 \in D[X]$ primitivo. % [cite: 307]
...
\begin{observacion}{Unicidad del contenido} % [cite: 326]
    El concepto de contenido de un polinomio es único salvo producto por unidades de $D$ ($D^\times$). % [cite: 326]
    En efecto, si $a, b \in C(P)$, entonces $P = a P_1 = b P_2$ con $P_1, P_2 \in D[X]$ primitivos. % [cite: 310, 312]
    ...
    Por tanto, existe una unidad $u \in D^\times$ tal que $b = a u$. % [cite: 326]
\end{observacion}

\begin{lema}{Lema de Gauss} % [cite: 327]
    Si $K$ es el cuerpo de fracciones de un DFU $D$, y $P, Q \in K[X] \setminus \{0\}$, entonces: % [cite: 329]
    $$C(P \cdot Q) = C(P) \cdot C(Q)$$ % [cite: 329]
    En particular, el producto de dos polinomios primitivos es primitivo (salvo unidad). % [cite: 330, 334]
\end{lema}
...
\begin{proof}
    ...
    Bastará con demostrar que el producto $P_1 Q_1$ es primitivo, ya que eso implicaría automáticamente que $ab \in C(PQ)$. % [cite: 335]
    
    Procedemos por reducción al absurdo. Supongamos que $P_1 Q_1$ no es primitivo. Entonces existe un elemento primo $p \in D$ que divide a todos los coeficientes de $P_1 Q_1$. % [cite: 335]
    Sean $P_1 = a_0 + a_1 X + \dots + a_n X^n$ y $Q_1 = b_0 + b_1 X + \dots + b_m X^m$. % [cite: 335]
    Como $P_1$ y $Q_1$ son primitivos, $\operatorname{mcd}(a_i) = 1$ y $\operatorname{mcd}(b_j) = 1$. Por lo tanto, $p$ no puede dividir a todos sus coeficientes. % [cite: 335]
    ...
    $$\dots + a_{i-1}b_{j+1} + a_i b_j + a_{i+1}b_{j-1} + \dots$$ % [cite: 335]
    Por nuestra elección minimal de $i$ y $j$, el primo $p$ divide a todos los sumandos anteriores a $a_i b_j$ (porque $p \mid a_k$ para $k < i$) y a todos los sumandos posteriores (porque $p \mid b_l$ para $l < j$). % [cite: 335]
    Sin embargo, $p \nmid a_i$ y $p \nmid b_j$, y como $p$ es primo, $p \nmid a_i b_j$. % [cite: 335]
    Por tanto, $p$ no puede dividir a la suma total, lo que es una flagrante contradicción con la suposición de que $p$ dividía a todos los coeficientes de $P_1 Q_1$. % [cite: 335]
\end{proof}

\begin{lema}{Lema 3.6} % [cite: 336]
    Sea $D$ un DFU, $K$ su cuerpo de fracciones y $P \in D[X]$, $Q \in K[X]$ polinomios mónicos. % [cite: 339]
    Si $Q$ divide a $P$ en el anillo $K[X]$, entonces $Q \in D[X]$ y $Q$ divide a $P$ en $D[X]$. % [cite: 339, 340]
\end{lema}

\begin{proof}
    Dado que $Q$ divide a $P$ en $K[X]$, existe un polinomio $R \in K[X]$ tal que $P = Q \cdot R$. % [cite: 341]
    Como $P$ es mónico y está en $D[X]$, sus coeficientes son coprimos (el coeficiente principal es 1, que solo es divisible por unidades). Por tanto, $P$ es primitivo y $C(P) = 1$. % [cite: 341]
    Además, como $P$ y $Q$ son mónicos, necesariamente $R$ debe ser también un polinomio mónico. % [cite: 341]
    
    Aplicamos el Lema de Gauss a la igualdad $P = Q \cdot R$: % [cite: 348]
    $$1 = C(P) = C(Q \cdot R) = C(Q) \cdot C(R)$$ % [cite: 342]
...
\begin{proof}
    ...
    Dividiremos esta demostración en dos fases lógicas. Primero demostraremos que $\Phi_n \in K[X]$ (donde $K$ es el cuerpo de fracciones de $\mathbb{Z}_p$) razonando por inducción fuerte sobre $n$. %
    
    \begin{itemize}
        \item \textbf{Caso base ($n=1$):} $\Phi_1(X) = X - 1$. Como los coeficientes $1$ y $-1$ pertenecen a cualquier cuerpo primo, trivialmente $\Phi_1 \in K[X]$. %
        \item \textbf{Paso inductivo:} Supongamos que la proposición es cierta para todo entero $d < n$. Esto es, para todo divisor $d$ de $n$ estrictamente menor que $n$, se cumple que $\Phi_d \in K[X]$. %
        Despejando el término $d=n$ del producto obtenido en el apartado anterior, tenemos: %
        $$X^n - 1 = \left( \prod_{\substack{d \mid n \\ d < n}} \Phi_d(X) \right) \cdot \Phi_n(X)$$ %
        Definamos $G(X) = \prod_{d \mid n, d < n} \Phi_d(X)$. Por la hipótesis de inducción, al ser un producto de polinomios en $K[X]$, el propio $G(X)$ pertenece a $K[X]$. Además, como cada $\Phi_d$ es mónico por definición, $G(X)$ también es un polinomio mónico. %
        
        Realizamos la división euclídea de $X^n - 1$ entre $G(X)$ dentro del anillo $K[X]$. El algoritmo de la división nos garantiza que existen únicos polinomios $C(X), R(X) \in K[X]$ tales que: %
        $$X^n - 1 = G(X) C(X) + R(X) \quad \text{con } \operatorname{gr}(R) < \operatorname{gr}(G)$$ %
        Sin embargo, operando en el cuerpo de descomposición (un cuerpo más grande), sabemos que la división es exacta y el cociente es $\Phi_n(X)$. Por la unicidad del algoritmo de división, el cociente y el resto deben ser los mismos independientemente de si operamos en $K$ o en su extensión. Por tanto, el resto $R(X)$ es obligatoriamente el polinomio nulo y el cociente $C(X) = \Phi_n(X)$ debe tener sus coeficientes en $K$. Concluimos que $\Phi_n \in K[X]$. %
    \end{itemize}
    
    A continuación, fortalecemos el resultado para demostrar que realmente caen en $\mathbb{Z}_p[X]$, dependiendo de la característica: %
    \begin{itemize}
        \item \textbf{Si $p > 0$:} El cuerpo primo $K$ es exactamente $\mathbb{Z}_p$. Al no haber denominadores, el anillo y su cuerpo de fracciones coinciden. Hemos probado que $\Phi_n \in K[X] = \mathbb{Z}_p[X]$. %
        \item \textbf{Si $p = 0$:} El anillo es $\mathbb{Z}_0 = \mathbb{Z}$ y su cuerpo de fracciones es $K = \mathbb{Q}$. Hasta ahora sabemos que $\Phi_n \in \mathbb{Q}[X]$. %
        Consideremos la igualdad $X^n - 1 = G(X) \cdot \Phi_n(X)$. %
        Sabemos que $X^n - 1 \in \mathbb{Z}[X]$. Además, tanto $G(X)$ como $\Phi_n(X)$ son polinomios mónicos con coeficientes en $\mathbb{Q}$. %
        Aplicamos el \textbf{Lema 3.6} (corolario del Lema de Gauss): si un polinomio mónico en $\mathbb{Q}[X]$ divide a un polinomio en $\mathbb{Z}[X]$, entonces el divisor pertenece necesariamente a $\mathbb{Z}[X]$. %
        Dado que $\Phi_n(X)$ divide a $X^n - 1$ en $\mathbb{Q}[X]$, deducimos que $\Phi_n(X) \in \mathbb{Z}[X]$. %
    \end{itemize}
