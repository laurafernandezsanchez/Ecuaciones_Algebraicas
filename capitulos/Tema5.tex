...
\begin{proof}
    Como $L/K$ es de Galois, del Teorema 5.10 se deduce que $L/L\cap E$ también es de Galois y de la Proposición 5.9 que lo es $LE/E$. Que la primera es finita es obvio y que lo sea la segunda es consecuencia de la Proposición 1.18.
    ...
    Para ver que $f$ es suprayectiva ponemos $H=\operatorname{Im} f\subseteq \operatorname{Gal}(L/L\cap E)$. Entonces $L\cap E=(L\cap E)^{\circ\circ}=\operatorname{Gal}(L/L\cap E)^{\circ}\subseteq H^{\circ}$. 
    Sea $\alpha\in H^{\circ}$. Entonces para todo $\sigma\in \operatorname{Gal}(LE/E)$ se verifica $\sigma(\alpha)=f(\sigma)(\alpha)=\alpha$, pues $f(\sigma)\in H$. Esto demuestra que $\alpha\in \operatorname{Gal}(LE/E)^{\circ}=E^{\circ\circ}=E$, pues $LE/E$ es de Galois.
     Como esto se verifica para cada $\alpha\in H^{\circ}$ y $H^{\circ}\subseteq L$ deducimos que $H^{\circ}\subseteq L\cap E=(L\cap E)^{\circ\circ}$. Por tanto, $H^{\circ}=(L\cap E)^{\circ\circ}$. Como $L/L\cap E$ es de Galois, concluimos que $\operatorname{Im} f=H=(L\cap E)^{\circ\circ\circ}=\operatorname{Gal}(L/L\cap E)$, o sea $f$ es suprayectiva.
\end{proof}
