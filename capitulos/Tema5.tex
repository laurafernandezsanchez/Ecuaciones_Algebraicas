...
Recordemos que si $L/K$ es una extensión de cuerpos, entonces el grupo de Galois de $L/K$ es el grupo $\operatorname{Gal}(L/K)$ formado por los automorfismos de $L/K$, es decir, los $K$-automorfismos de $L$. ]% 4]

\begin{ejemplo}{}
    \begin{enumerate}[label=(\arabic*)]
        \item Claramente $\operatorname{Gal}(K/K)=1$, pero no son éstas las únicas extensiones con grupo de Galois trivial. ]% 5]
        Por ejemplo, si $a$ es un número racional positivo que no es el cubo de un número racional, entonces $p=X^{3}-a$ es irreducible en $\mathbb{Q}[X]$. ]% 6]
        Las raíces de $p$ son $\alpha=\sqrt[3]{a}$, $\omega\alpha$ y $\omega^{2}\alpha$, donde $\omega$ es una raíz tercera primitiva de la unidad. ]% 7]
        Como $\omega$ no es un número real, la única raíz de $p$ que pertenece a $K(\alpha)$ es $\alpha$ y por tanto $\operatorname{Gal}(K(\alpha)/K)=1$ (¿por qué?). ]% 8]
        
        \item Si $L/K$ es una extensión de grado 2 y $\operatorname{car}(K)\ne 2$, entonces $\operatorname{Gal}(L/K)$ tiene dos elementos. ]% 9]
        En efecto, si $\alpha\in L\setminus K$, entonces $L=K(\alpha)$ y por tanto $p=\operatorname{Min}_{K}(\alpha)$ tiene grado 2. Pongamos $p=X^{2}+aX+b=\left(X+\frac{a}{2}\right)^{2}+b-\frac{a^2}{4}$ y sean $\beta=\alpha+\frac{a}{2}$ y $c=\frac{a^{2}}{4}-b$. ]% 10]
        Entonces $q=\operatorname{Min}_{K}(\beta)=X^{2}-c$ y $L=K(\beta)=K(\sqrt{c})$. Como las raíces de $q$ son $\pm\beta$, si $\sigma\in \operatorname{Gal}(L/K)$ entonces $\sigma(\beta)=\pm\beta$, con lo que efectivamente $\operatorname{Gal}(L/K)$ tiene dos elementos. ]% 11]
        (¿Seguro que tiene dos?). 
        En particular, $\operatorname{Gal}(\mathbb{C}/\mathbb{R})$ tiene orden 2 y de hecho está formado por la identidad y la conjugación. ]% 13]
        
        \item Como un automorfismo de $\mathbb{R}$ ha de ser una aplicación creciente (¿por qué?), necesariamente $\operatorname{Gal}(\mathbb{R}/\mathbb{Q})=1$ y por tanto $\operatorname{Gal}(\mathbb{R}/K)=1$ para todo subcuerpo $K$ de $\mathbb{R}$ (¿por qué?). ]% 14]
        De hecho, el único automorfismo de $\mathbb{R}$ es la identidad (¿por qué?). ]% 15]
        
        \item Sean $K=\mathbb{Q}(\sqrt{2},\sqrt{3})$ y $\sigma\in \operatorname{Gal}(K/\mathbb{Q})$. Entonces $\sigma(\sqrt{2})=\pm\sqrt{2}$ y $\sigma(\sqrt{3})=\pm\sqrt{3}$ y por tanto $\operatorname{Gal}(K/\mathbb{Q})$ tiene a lo sumo 4 elementos. ]% 16]
        De hecho $\operatorname{Gal}(K/\mathbb{Q})$ tiene exactamente cuatro elementos. En efecto, en el Ejemplo (2) hemos visto que $\operatorname{Gal}(\mathbb{Q}(\sqrt{2})/\mathbb{Q})$ tiene 2 elementos. ]% 17]
        Por otro lado $\sqrt{3}\notin\mathbb{Q}(\sqrt{2})$ (ver Ejercicio 1.11 del Capítulo 1). ]% 18]
        Por tanto, $K/\mathbb{Q}(\sqrt{2})$ es una extensión separable (¿por qué?) de grado 2, con lo que cada uno de los dos elementos de $\operatorname{Gal}(\mathbb{Q}(\sqrt{2})/\mathbb{Q})$ tiene dos extensiones a un homomorfismo de $K$ en una clausura algebraica de $K$ que, como además $K/\mathbb{Q}$ es normal (¿por qué?), estas dos extensiones son elementos de $\operatorname{Gal}(K/\mathbb{Q})$. Por tanto $\operatorname{Gal}(K/\mathbb{Q})$ tiene cuatro elementos: $\sigma_{++}, \sigma_{+-}, \sigma_{-+}, \sigma_{--}$ dados por $\sigma_{ab}(\sqrt{2})=a\sqrt{2}$ y $\sigma_{ab}(\sqrt{3})=b\sqrt{3}$. ]% 19]
        
        \item Sea $\xi$ una raíz $n$-ésima primitiva de la unidad y sea $L=K(\xi)/K$ una extensión ciclotómica. ]% 20]
        Si $\sigma\in \operatorname{Gal}(L/K)$, entonces $\sigma(\xi)=\xi^{i}$ para algún entero $i$ coprimo con $n$, y $\sigma$ está completamente determinada por el resto de $i$ módulo $n$. ]% 21]
        Por tanto, tenemos una aplicación $\psi:\operatorname{Gal}(L/K)\rightarrow\mathbb{Z}_{n}^{*}$ que asocia $\sigma\in \operatorname{Gal}(L/K)$ con la única clase en $\mathbb{Z}_{n}^{*}$ que contiene a $i$ (con $\sigma(\xi)=\xi^{i}$). ]% 22]
        Entonces % 23, 24, 25, 26] 
        es un homomorfismo inyectivo de grupos (comprobarlo) y por tanto $\operatorname{Gal}(L/K)$ es isomorfo a un subgrupo de $\mathbb{Z}_{n}^{*}$. En particular, el grupo de Galois de toda extensión ciclotómica es abeliano. ]% 27]
        Si además $K=\mathbb{Q}$, entonces $\operatorname{Min}_{\mathbb{Q}}(\xi)=\Phi_{n}$, el $n$-ésimo polinomio ciclotómico (Teorema 3.9). ]% 28]
        Por tanto, para cada $i$ coprimo con $n$ existe un elemento $\sigma\in \operatorname{Gal}(L=\mathbb{Q}(\xi)/\mathbb{Q})$ con $\sigma(\xi)=\xi^{i}$. En otras palabras, $\operatorname{Gal}(\mathbb{Q}(\xi)/\mathbb{Q})$ es isomorfo a $\mathbb{Z}_{n}^{*}$ y un isomorfismo $\tau:\mathbb{Z}_{n}^{*}\rightarrow \operatorname{Gal}(\mathbb{Q}(\xi)/\mathbb{Q})$ viene dado asociando $i\in\mathbb{Z}_{n}^{*}$ con el único automorfismo $\tau_{i}$ de $\mathbb{Q}(\xi)$ tal que $\tau_{i}(\xi)=\xi^{i}$. ]% 29]
    \end{enumerate}
\end{ejemplo}
...
Si $L/K$ es una extensión algebraica y $\overline{L}$ una clausura algebraica de $L$, entonces podemos ver cada elemento de $\operatorname{Gal}(L/K)$ como un elemento de $S_{1}^{L}=\{\sigma:L\rightarrow\overline{L} \mid \sigma|_{K}=1_{K}\}$. ]% 31]
